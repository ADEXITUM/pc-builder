...
const dom = {
  ...
  additionalsContainer: document.querySelector("#additionalsContainer"),
  addAdditionalButton: document.querySelector("#addAdditionalButton"),
};
...
function bindEvents() {
  ...
  dom.addAdditionalButton.addEventListener("click", () => {
    addAdditionalEditorRow({});
    updateTotalPrice();
  });
}
...
function loadBuilds() {
  try {
    ...
    // migrate: add purchased:false if missing, and additionals if missing
    parsed.forEach((build) => {
      COMPONENT_ORDER.forEach(({ key }) => {
        if (build.components[key] && build.components[key].purchased === undefined) {
          build.components[key].purchased = false;
        }
      });
      if (!Array.isArray(build.additionals)) {
        build.additionals = [];
      }
    });
...
function renderAdditionalsEditor(items) {
  dom.additionalsContainer.innerHTML = "";
  items.forEach((item) => addAdditionalEditorRow(item));
}

function addAdditionalEditorRow(item = {}) {
  const row = document.createElement("div");
  row.className = "additional-editor";
  row.innerHTML = `
    <div class="additional-editor-fields">
      <label class="field">
        <span>Название</span>
        <input type="text" data-field="name" placeholder="Название" value="${escapeAttribute(item.name || "")}">
      </label>
      <label class="field field-narrow">
        <span>Цена</span>
        <input type="number" data-field="price" min="0" step="1" placeholder="0" value="${item.price || ""}">
      </label>
      <label class="field">
        <span>Ссылка</span>
        <input type="url" data-field="url" placeholder="https://..." value="${escapeAttribute(item.url || "")}">
      </label>
    </div>
    <button class="button button-ghost additional-remove" type="button" title="Удалить">×</button>
  `;
  row.querySelector(".additional-remove").addEventListener("click", () => {
    row.remove();
    updateTotalPrice();
  });
  dom.additionalsContainer.appendChild(row);
}
...
function renderBuildList() {
  ...
  state.builds.forEach((build) => {
    ...
    const additionals = build.additionals || [];
    ...
    card.innerHTML = `
      <div class="build-card-top">
        ...
        <div class="build-totals">
          ...
          ${remaining < total ? `<div class="build-remaining ${allPurchased ? "all-purchased" : ""}">
      ...
      <div class="spec-list">
        ...
        ${additionals.length ? `<div class="spec-divider"></div>` : ""}
        ${additionals.map((item, index) => renderAdditionalRow(item, index)).join("")}
      </div>
    ...
    card.addEventListener("click", (event) => {
      ...
      if (checkbox) {
        ...
        if (key && build.components[key]) {
          build.components[key].purchased = checkbox.checked;
        } else {
          const idx = Number(checkbox.dataset.additional);
          if (!isNaN(idx) && build.additionals[idx]) {
            build.additionals[idx].purchased = checkbox.checked;
          }
        }
        persistBuilds();
        renderBuildList();
...
function renderAdditionalRow(item, index) {
  const purchased = item.purchased === true;
  return `
    <div class="spec-row ${purchased ? "is-purchased" : ""}">
      <div class="spec-left spec-left-additional">+</div>
      <div class="spec-main">
        <div class="spec-name">${escapeHtml(item.name)}</div>
        ${item.url ? `<a class="link" href="${escapeAttribute(item.url)}" target="_blank" rel="noreferrer">Ссылка</a>` : ""}
      </div>
      <div class="spec-price-col">
        <div class="spec-price ${purchased ? "purchased-price" : ""}">${item.price ? formatPrice(item.price) : "—"}</div>
        <label class="purchased-label" title="${purchased ? "Куплено" : "Отметить как купленное"}">
          <input class="purchased-checkbox" type="checkbox" data-additional="${index}" ${purchased ? "checked" : ""}>
          <span class="purchased-tick"></span>
        </label>
      </div>
    </div>
...
function handleSubmit(event) {
  ...
  const payload = {
    ...
    additionals: collectAdditionalsFromForm(),
  };
  ...
  if (index >= 0) {
    ...
    // preserve purchased state for additionals by matching index
    const existingAdditionals = state.builds[index].additionals || [];
    payload.additionals.forEach((item, i) => {
      if (existingAdditionals[i]) {
        item.purchased = existingAdditionals[i].purchased ?? false;
      }
    });
...
function collectAdditionalsFromForm() {
  return Array.from(dom.additionalsContainer.querySelectorAll(".additional-editor")).map((row) => ({
...
function updateTotalPrice() {
  const componentsTotal = Object.values(collectComponentsFromForm()).reduce((sum, item) => sum + Number(item.price || 0), 0);
  const additionalsTotal = collectAdditionalsFromForm().reduce((sum, item) => sum + Number(item.price || 0), 0);
  dom.totalPrice.textContent = formatPrice(componentsTotal + additionalsTotal);
}
...
function fillForm(build) {
  ...
  COMPONENT_ORDER.forEach((component) => {
    const values = build?.components?.[component.key] || { name: "", price: 0, url: "", purchased: false };
    const root = getComponentNode(component.key);
  ...
  renderAdditionalsEditor(build?.additionals || []);
  updateTotalPrice();
}
...
function duplicateBuild(id) {
  ...
  (copy.additionals || []).forEach((item) => { item.purchased = false; });
...
function calculateTotal(build) {
  const componentsTotal = COMPONENT_ORDER.reduce((sum, component) => sum + Number(build.components[component.key]?.price || 0), 0);
  const additionalsTotal = (build.additionals || []).reduce((sum, item) => sum + Number(item.price || 0), 0);
  return componentsTotal + additionalsTotal;
}

function calculateRemaining(build) {
  const componentsRemaining = COMPONENT_ORDER.reduce((sum, component) => {
    const item = build.components[component.key];
    if (!item?.name || item.purchased) return sum;
    return sum + Number(item.price || 0);
  }, 0);
  const additionalsRemaining = (build.additionals || []).reduce((sum, item) => {
    if (!item.name || item.purchased) return sum;
    return sum + Number(item.price || 0);
  }, 0);
  return componentsRemaining + additionalsRemaining;
}
